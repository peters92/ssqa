...
class MiniBatchLoader():
    ...
    def reset(self):
        ...
        if self.shuffle:
            # for ixs in self.bins.itervalues(): #Python2 dict iteration
            for ixs in iter(self.bins.values()): #Python3 dict iteration
                random.shuffle(ixs)
        ...
        self.batch_pool = []
        # for l, ixs in self.bins.iteritems(): #Python2 dict iteration
        for l, ixs in iter(self.bins.items()): #Python3 dict iteration
            n = len(ixs)
            # k = n/self.batch_size if n % self.batch_size == 0 else n/self.batch_size+1 #Python2 floor division
            k = n/self.batch_size if n % self.batch_size == 0 else n//self.batch_size+1 #Python3 division
            ixs_list = [(ixs[self.batch_size*i:min(n, self.batch_size*(i+1))],l) for i in range(k)]
            self.batch_pool += ixs_list
        print(self.batch_pool)
    ...
    def next(self):
        ...
        for n, ix in enumerate(ixs):
            ...
            for it,cc in enumerate(cand):
                index = [ii for ii in range(len(doc_w)) if doc_w[ii] in cc] # Get indices of candidate word in the document
                m_c[n,index] = 1
                c[n,index,it] = 1
